...
//Returns the free index in the bit map/the block # that the disk block will be stored at in the file
 int setBitmap(char *dir, char value) {
 	printf("setBitmap\n");
 	printf("Dir: %s\n", dir);
...
//Create directory
 int createDirectory(char *dir, int ind) {
	...
	//Allocate space for a directory, and set the number of files in that directory to 0
 	...
 	printf("Create dir directory fwrite\n");
 ...
 */
 	...
 	struct fuse_file_info *fi)
 {
	...
	int findDirReturn = findDirectory(directory);
 	...
 	fseek(file, sizeof(*dir) * findDirReturn, SEEK_SET);
 	fread(dir, sizeof(*dir), 1, file);

 	//get the starting block for the inode
 	long fileStartBlock = dir->files[findDirReturn].nStartBlock;
 	...
 	for (i = 0; i < inode->children; i++) {
 		...
 		int j;
 		for (j = strlen(buf); j < strlen(diskBlock->data); j++)
 			buf[j] = diskBlock->data[j];
 ...
 */
 	...
 	off_t offset, struct fuse_file_info *fi)
 {
 	...
 	//Get the last element's start block in the inode array of disk blocks
 	//I get the last one because I assume that all other disk blocks are full
 	...
 	long spaceLeft = size;
 	//While there is still data that we need to write to .disk
 	while (spaceLeft > 0) {
 		long sizeToWrite = spaceLeft;
 		printf("Size to write: %ld\n", sizeToWrite);
 		spaceLeft -= strlen(diskBlock->data);
 		if (strlen(diskBlock->data) == 0)
 			spaceLeft -= 508;
 		if (spaceLeft < 0)
 			spaceLeft = 0;

 		printf("Data size: %ld\n After subtraction: %ld\n", strlen(diskBlock->data), spaceLeft);
 		int i;
 		//Write 1's to the disk block
 		...
 		//If the disk block is full and we still have more data to write
 		//Then create a new disk block
 		if ( (strlen(diskBlock->data) == MAX_DATA_IN_BLOCK) && (spaceLeft > 0) ) {
 			//Write the current diskblock to disk
 			printf("Writing diskBlock\n");
 			fseek(file, sizeof(*diskBlock) * diskStartBlock, SEEK_SET);
 			fwrite(diskBlock, sizeof(*diskBlock), 1, file);
 			printf("Freeing diskBlock\n");
 			free(diskBlock);



 			//Create a new disk block
